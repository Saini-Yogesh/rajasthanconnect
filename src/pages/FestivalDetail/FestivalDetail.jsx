...
export default function FestivalDetail() {
  ...
  const relatedCities = allCities.filter(c => festival?.related_city_ids?.includes(c.id));
  const relatedFoods = allFoods.filter(f => festival?.related_food_ids?.includes(f.id));
  const relatedCulture = allCulture.filter(c => festival?.related_culture_ids?.includes(c.id));
        ...
        <div className="festivalGrid">
          ...
          <div className="festLeft">
            <div 
              ...
              style={{ backgroundImage: `url(${festival.image_urls?.[0] || 'https://images.unsplash.com/photo-1605647540924-852290f6b0d5?auto=format&fit=crop&w=800&q=80'})` }}
            />
            ...
            {(relatedCities.length > 0 || relatedFoods.length > 0 || relatedCulture.length > 0) && (
                ...
                <div className="festivalConnectionsLinks">
                  {relatedCities.length > 0 && (
                    <div className="festConnRow">
                      <span className="festConnLabel">Grand Processions In:</span>
                      <div className="festConnList">
                        {relatedCities.map(c => (
                          <Link to={`/cities/${c.id}`} className="festConnLink" key={c.id}>
                            {c.name}
                          </Link>
                        ))}
                      </div>
                  ...
                  {relatedFoods.length > 0 && (
                    <div className="festConnRow">
                      <span className="festConnLabel">Festive Culinary Specialties:</span>
                      <div className="festConnList">
                        {relatedFoods.map(f => (
                          <Link to={`/foods/${f.id}`} className="festConnLink" key={f.id}>
                            {f.title}
                          </Link>
                        ))}
                      </div>
                  ...
                  {relatedCulture.length > 0 && (
                    <div className="festConnRow">
                      <span className="festConnLabel">Folk Dance & Attire:</span>
                      <div className="festConnList">
                        {relatedCulture.map(c => (
                          <Link to="/history-culture" className="festConnLink" key={c.id}>
                            {c.title} ({c.category})
                          </Link>
                        ))}
                      </div>
