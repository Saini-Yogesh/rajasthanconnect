...
router.post('/plan-trip', async (req, res) => {
  try {
    ...
    console.log(`Generating itinerary: ${days} days, budget ${budget} INR, starting in ${startingCity}...`);
    const itinerary = await gemini.generateItinerary({ days, budget, startingCity, interests });
    res.json(itinerary);
  } catch (err) {
...
router.post('/chat', async (req, res) => {
  try {
    ...
    console.log(`Querying AI Chatbot: "${messageHistory[messageHistory.length - 1].content}"`);
